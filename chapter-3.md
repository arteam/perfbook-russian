...
**Быстрый вопрос 3.3:** Действительно ли дизайнеры процессоров сильно уменьшили накладные расходы промахов кэша?
...
**Быстрый вопрос 3.6:** Безусловно, проектировщиков аппаратного обеспечения можно убедить улучшить эту ситуацию! Почему они мирятся с такой ужасной производительность для этих операций длиной в одну инструкцию?
...
Значение коэффициентов в таблице 3.1 критично важны, так как они ограничивают эффективность параллельного приложения. Для того чтобы увидеть это, предположим, что праллельное приложение использует CAS операцию для коммуникации между потоками. Эта CAS операция обычно будет нести с собой промах кэша (предпологая, что потоки взаимодействуют в основном между собой, чем внутри друг друга). Предположим, что единица работы, соответствующая каждой CAS операции занимает 300 нс, что является достаточным временем для вычисления нескольких трансцендентных функций с плавающей запятой. Тогда примерно половина времени выполнения работы будет занято выполнением CAS операции! Это в свою очередь значит, что двухпроцессорная система, запускающая такую параллельную программу будет работать не быстрее, чем последовательная реализация на одном процессоре.
...
**Быстрый вопрос 3.9** Учитывая, что коммуникация в распределенных системах настолько дорогая, почему еще кто-то работает с ними?

Урок должен быть вполне ясным: параллельные алгоритмы обязыны быть явно спроектированы для запуска на практически независимых потоках. Чем меньше потоки взаимодействуют между собой, как путем атомарных операций, блокировок, так и путем явных сообщений, тем лучше будут производительность и масштабируемость приложения. Если коротко, достижение отличной параллельной производительности значит обращение к позорно параллельным алгоритмам и реализациями: 
...
**Быстрый вопрос 3.10** Хорошо, если мы собираемся применить техники распределенного программирования на паралелльные программы с разделяемой памятью, почему не использовать их всегда и вообще убрать разделение памяти между потоками?

Подводя итоги:

1. Хорошие новости в том, что мультиядерные системы недороги и широко доступны.

2. Еще более хорошие новости в том, что накладные расходы многих операций синхронищации меньше, чем они были на параллельных системах в начале 2000-х.
