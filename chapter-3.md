...
Большинство людей имеют интуитивное понимание того, что передача сообщений между системами более дорога, чем выполнение простых вычислений в рамках одной системы. Однако, не всегда ясно, что коммуникация между потоками в рамках одной системы с разделенной памятью может быть тоже вполне дорогой. Эта глава рассматривает цену синхронизации и коммуникации в рамках такой системы. Эти несколько страниц все лишь покажут вершину айсберга дизайна параллельного аппаратного обеспечения с разделенной памятью. Читатели, желающие получить больше деталей, могут начать с последного издания классической книги Хеннесси и Паттерсона [HP95].

**Быстрый вопрос 3.1:**(../master/quick-quiz-3.md#Быстрый-вопрос-31)  Зачем программистам параллельных систем учить низкоуровненевые свойства аппартаного обеспечения? Не будет ли проще, лучше, и более обще отстаться на более высоком уровне абстракции?

## 3.1 Обзор

Беззаботные читатели спецификаций компьютерных систем могут прийти к выводу, что производительность процессоров - это забег на чистой дорожке, как показано на рисунке 3.1, где победу всегда одерживает быстрейший.

![Рисунок 3.1](../master/cartoons/trackmeet.png?raw=true)

Рисунок 3.1 Производительность процессоров как забег
...
Как работал в начале 1980-х типичный микропроцессор: он выбирал инструкцию, декодировал ее и затем выполнял. В итоге требовалось *как минимум* 3 такта, чтобы обработать одну инструкцию перед переходом к следующей. На контрасте, типичный процессор конца 1990-х - начала 2000-х выполняет много инструкций одновременно, используя глубокий "конвейер" для управления потоком инструкций внутри процессора. Эти современные аппаратные возможности могут сильно улучшить производительность, как показано на рисунке 3.2.

![Рисунок 3.2](../master/cartoons/old_man_and_brat.png?raw=true)
...
Однако, предположим, что мы имеем либо программу с большим количеством коротких циклов или объектно-ориентированную программу с большим количеством виртуальных объектов, ссылающихся на реальные объекты, которые имеют различные реализации для часто вызываемых методов. В этих случаях для процессора сложно или даже невозможно предсказать какая ветка будет исполнена. Процессор, следовательно, вынужден либо простаивать, ожидая, что поток выполнения продвинется до момента, когда можно точно знать какая ветка будет исполнена или догадываться. И для программ с непредсказуемым потоком управления - часто догадываться неправильно. Неправильные догадки могут быть очень дорогими, так как процессор вынужден выбросить все результаты инструкций, которые были выполнены спекулятивно. Более того, в независимости от того, простаивает процессор или догадывается, конвейер будет пустым и требовать наполнения. Это приведет к простоям, которые могут значительно уменьшить производительность, как ярко показано на рисунке 3.3. 
...
В 1980-х годах микропроцессор часто мог загрузить значение из памяти быстрее, чем выполнить инструкцию. В 2006 году микропроцессор имеет возможность выполнять сотни или даже тысячи интсрукций за время единичного доступа к памяти. Это несоотвествие происходит из-за того, что закон Мура увеличил производительность процессоров с намного большей степенью, чем производительность памяти (стоит учитывать степень, с которой вырос размер памяти). Для примера, типичный компьютер 1970-x мог иметь 4 килобайта (да, килобайта, не мегабайта, не говоря уже о гигабайтах) оперативной памяти с однотактовым доступом. В 2008 году проектировщики процессоров все еще могут создать 4 килобайтную память с однотактовым доступом, даже на системах с тактовами частотами в несолько гигагерц. Такая память часто создается, только она теперь называется "кэш 0 уровня" и она довольно больше, чем 4 килобайта.

Несмотря на то что большие кэши, доступные на современных микропроцессорах, могут сделать много вещей для борьбы с задержками доступа к памяти, для успешной работы они требуют высоко предсказуемых паттернов доступа. К сожалению, типичные операции, такие как обход связного списка, высоко непредсказуемы. В конце концов, если бы паттерны были предсказуемы, мы, программисты, не беспокоилсь бы об указателях, верно?
...
До сих пор мы рассматривали только препятствия, которые могут возникнуть во время исполнения процессором однопоточного кода. Мультипоточность предоставляет дополнительные препятствия процессору, как описано в следующих разделах.

### 3.1.3 Атомарные операции

Одним их таких препятствий являются атомарные операции. Сама идея атомарной операции в некотром роде конфликтует со структурой "сборочной линии" современного процессорного конвеера. К чести проектировщиков аппаратного обеспечения, современные процессоры используют некоторые количество крайне умных трюков, чтобы заставить эти операции *выглядеть* атомарными, несмотря на то, что они на самом дела выполняются по частям. Но даже так, все равно существуют случаи, когда конвеер обязан быть задержан или даже сброшен, для того чтобы определенная атомарная операциия выполнилась корректно. 
...
К содалению, атомарные операции обычно применяются только к единичным частям данных. Так как многие параллельные алгоритмы требуют поддержки ограничений порядка выполнения между обновлениями нескольких элементов данных, то большинство процессоров предоставляют барьеры памяти. Эти барьеры также являются препятствиями, который подрывают производительность, как описано в следующем разделе.

**Быстрый вопрос 3.2:**(../master/quick-quiz-3.md#Быстрый-вопрос-32) Какие типы машин разрешали бы атомарные операции над несколькими элементами данных?
...
Барьеры памяти будут рассмотрены более глубоко в разделе 14.2 и приложении C. Сейчас же рассмотрим следующую простую критическую секцию на основе лока.
...
**Быстрый вопрос 3.3:**(../master/quick-quiz-3.md#Быстрый-вопрос-33) Действительно ли дизайнеры процессоров сильно уменьшили накладные расходы промахов кэша?
...
Промах кэша можно воспринимать как операцию ввода/вывода между процессорами. И если рассматривать ее в этом виде, то это одна из наиболее дешевых операциий такого рода. Операции ввода/вывода, включащие общение по сети, работу с диском или (что еще хуже) с людьми, ставят намного более большие препятствия, чем их внутренние аналоги, которые мы обсуждали в предыдущих разделах. Это показано на рисунке 3.8.  
...
Это одно из различий между параллелизмом в системах с разделяемой памятью и распеределенных системах. Системы с разделяемой памятью работают с препятствиями не больше, чем  промаха кэша, в то время как распеределенные системы обычно несут с собой большие задержки сетевого взаимодействия. В обоих случаях, соответствующие задержки могут воспрниматься как цена коммуникации - цену, которую не нужно бы было платить в последовательной программе. Следовательно, отношение между накладными расходами на коммуникации к реальной выполненной работе является ключевым параметром проектирования системы. Важная цель проектирования параллельного аппаратного обеспечения - это уменьшить это отношение настолько, чтобы достить соответствующих целей по производительности и масштабируемости. В свою очередь, как мы увидим в главе 6, важной целью является и уменьшение частоты дорогих операций, таких как промах кэша.
...
Этот раздел показывает актуальные накладные расходы препятствий для производительности, описанных в предущих разделах. Однако, для того, чтобы их понять, сначала необходимо бросить грубый взгляд на аппаратную системную архитектуру. Это и является темой следующего раздела.
...
**Быстрый вопрос 3.4:**(../master/quick-quiz-3.md#Быстрый-вопрос-34) Это *упрощенная* последовательность событий? Как она вообще может быть более сложной?

**Быстрый вопрос 3.5:**(../master/quick-quiz-3.md#Быстрый-вопрос-35) Зачем необходимо сбрасывать кэш-линию из кэша процессора 7?
...
| --------------------------- | -----------:| -----------:|
| Тактовая частота            | 0.6         | 1.0         |
| Лучший случай CAS           | 37.9        | 63.2        |
| Лучший случай лока          | 65.6        | 109.3       |
| Единичный промах кэша       | 139.5       | 232.5       |
| Единичный промах кэша в CAS | 306.0       | 510.0       |
| Коммуникация в кластере     | 3,000       | 5,000       |
| Глобальная коммуникация     | 130,000,000 | 216,000,000 |
...
Предыдущие раздела показали некоторые важные аппаратные препятствия при использовании конкурентности. Одно из жестких физических ограничений с которыми встречаются разработчики аппаратного обеспечения - это конечность скорости света. Как показано на рисунке 3.9, свет может проделать только 8-сантимеровый путь туда и обратно в вакууме за тактовой период в 1.8 ГГц. Это расстояние уменьшается до 3 сантиметров для тактового периода в 5 ГГц. Оба этих расстояния относительны малы по сравнению с размером современной компьютерной системы.

Что еще хуже, электоны в кремнии передвигаются от 3 до 30 раз медленнее, чем в вакууме, а типичные логические конструкции работет еще медленее. Например, ссылка на память может ожидать заверешение поиска в локальном кэше и только после этого запрос может быть передан остальной системе. Более того, для передачи электрических сигналов с одной кремниевой матрицы на другую требуются драйверы с относительно низкой скоростью и высокой мощностью. Например, они используются для коммуникации между процессором и оперативной памятью. 

**Быстрый вопрос 3.8:**(../master/quick-quiz-3.md#Быстрый-вопрос-38) Но отдельные электроны не перемещаются так быстро нигде, даже в проводниках! Скорость дрейфа электрона в проводнике на низких напряжениях, которая может быть найдена в полупроводниках, имеет величину порядка *миллиметра* в секунду. Где подвох?
...
Возможно, наиболее важное преимущество 3DI - это уменьшенная длины пути через систему, как показано на рисунке 3.11. 3-сантиметровая кремнивая матрица заменяется стеком из 4-х 1.5 сантиметровых матриц, что в теории уменьшает максимальный путь через систему в 2 раза, если каждый уровень достаточно тонок. К тому же, при должном внимании к проектированию и развертыванию, длинные горизонтальные элекрические соеденения (медленные и требующие большого количества энергии) могут быть заменены на короткие вертикальные электрические соеденения, которые быстры и более эффективны с точки зрения потребления мощности.
...
Хотя мультиядерные процессоры пришли в компьютерную индустрию внезапно, остается фактом, что параллельные системы с разделяемой памятью были коммерчески доступны более чем 25 лет. Это более чем достаточный срок для появления значительного параллельного программного обеспечение, и оно действительно появилось. Параллельные операционные системы вполне обычны как и параллельные библиотеки работы с потоками, параллельные системы управления базами данных и параллельное вычислительное программное обеспечение. Техника использования существующего параллельное программного обеспечения может пройти долгий путь до любого кризиса, с которым мы можем столкнуться.

Возможно, самый типичный пример - это праллельные системы управления базами данных. Вполне обычная ситуация, когда однопоточные программы, написанные на высокоуровнех скриптовых языках, работают с центральной базой данных конкурентно. В конечной высокопараллельной системе, только база данных в должна работать с параллелизмом. Очень красивый трюк, когда работает!

## 3.4 Последствия дизайна программного обеспечения

Значение коэффициентов в таблице 3.1 критично важны, так как они ограничивают эффективность параллельного приложения. Для того чтобы увидеть это, предположим, что праллельное приложение использует CAS операцию для коммуникации между потоками. Эта CAS операция обычно будет нести с собой промах кэша (предполагая, что потоки взаимодействуют в основном между собой, чем внутри друг друга). Предположим, что единица работы, соответствующая каждой CAS операции занимает 300 нс, что является достаточным временем для вычисления нескольких трансцендентных функций с плавающей запятой. Тогда примерно половина времени выполнения работы будет занято выполнением CAS операции! Это в свою очередь значит, что двухпроцессорная система, запускающая такую параллельную программу будет работать не быстрее, чем последовательная реализация на одном процессоре.

Ситуация еще хуже в случае распределенной системы, где задержка одной операции коммуникации может занимать тысячи или даже миллионны операций с плавающей запятой. Это показывает, насколько важно для операций коммуникации быть крайне редкими и обрабатывать большие объемы данных за раз.

**Быстрый вопрос 3.9:**(../master/quick-quiz-3.md#Быстрый-вопрос-39) Учитывая, что коммуникация в распределенных системах настолько дорогая, почему еще кто-то работает с ними?

Урок должен быть вполне ясным: параллельные алгоритмы обязаны быть явно спроектированы для запуска на практически независимых потоках. Чем меньше потоки взаимодействуют между собой, как путем атомарных операций, блокировок, так и путем явных сообщений, тем лучше будут производительность и масштабируемость приложения. Если коротко, достижение отличной параллельной производительности значит обращение к позорно параллельным алгоритмам и реализациями: 

* либо путем осторожного выбора структур данных и алгоритмов и использование существующих параллельных приложений и сред;
* или преобразованием проблемы в ту, для которой существует позорно параллельное решение.

**Быстрый вопрос 3.10:**(../master/quick-quiz-3.md#Быстрый-вопрос-310) Хорошо, если мы собираемся применить техники распределенного программирования на паралелльные программы с разделяемой памятью, почему не использовать их всегда и вообще убрать разделение памяти между потоками?

Подводя итоги:

1. Хорошие новости в том, что мультиядерные системы недороги и широко доступны.

2. Еще более хорошие новости в том, что накладные расходы многих операций синхронизации меньше, чем они были на параллельных системах в начале 2000-х.

3. Плохие новости в том, что накладные расходы промахов кэша все еще высоки, особенно в больших системах.

Оставшаяся часть этой книги описывает пути преодоления плохой новостей.

В деталях: глава 4 опишет некоторые низкоуровневые инструменты, используемые в параллельном программировании, глава 5 исследует проблемы и решения для параллельного подсчета и глава 6 обсудит дисциплины дизайна, которые продвигают проивзодительность и масштабируемость.
